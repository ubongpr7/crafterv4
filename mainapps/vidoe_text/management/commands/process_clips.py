...
#         return CompositeVideoClip([clip, box_clip, subtitle_clip])
    ...
    def add_subtitles_to_clip(
            ...
            box_position = (
                "center" if not x_offset else (clip.w // 2 - x_offset, clip.h - box_height - 2 * margin - safe_zone_offset)
            )
            subtitle_position = (
                "center" if not x_offset else (clip.w // 2 - x_offset, clip.h - box_height - 2 * margin + (box_height - text_height) / 2 - safe_zone_offset)
            )
            box_clip = box_clip.set_position(box_position)
            subtitle_clip = subtitle_clip.set_position(subtitle_position)
